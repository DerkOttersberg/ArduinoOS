...
// For accumulating full commands made of tokens (optional, if you want to parse multiple tokens)
#define MAX_TOKENS 5
...
void loop() {
  if (readTokenNonBlocking()) {
    // Command is ready, process tokens in 'tokens' array

    if (tokenCount > 0) {
      // Handle the first token as command
      if (strcmp(tokens[0], "run") == 0 && tokenCount >= 2) {
        startProcess(tokens[1]);
      } else if (strcmp(tokens[0], "kill") == 0 && tokenCount >= 2) {
        int pid = atoi(tokens[1]);
        killProcess(pid);
      } else if (strcmp(tokens[0], "memfree") == 0) {
        Serial.print("Free memory: ");
        Serial.print(getFreeSpace());
        Serial.println(" bytes");
      } else if (strcmp(tokens[0], "ls") == 0) {
        listFiles();
      } else if (strcmp(tokens[0], "readfile") == 0 && tokenCount >= 2) {
        byte buffer[256];
        if (readFile(tokens[1], buffer, 256)) {
          Serial.println((char*)buffer);
        }
      } else if (strcmp(tokens[0], "writefile") == 0 && tokenCount >= 3) {
        // For writefile, the 2nd token is filename, the rest is data joined
        // Here we just concatenate tokens 2..end with spaces (simple)
        String data = "";
        for (int i = 2; i < tokenCount; i++) {
          data += tokens[i];
          if (i < tokenCount - 1) data += " ";
        }
        createFile(tokens[1], (byte*)data.c_str(), data.length() + 1);
      } else {
        Serial.println("Unknown command.");
      }
    }
